...
export function initBrowserRag(lexiconPayload) {
  lexicon = lexiconPayload;
  entries = lexiconPayload?.entries ?? [];
}
...
export function retrieveByVector({
  ...
  if (!trimmedQuery) {
    return {
      retrievalMode: "browser-local-pseudo-vector",
      totalCandidates: 0,
      items: []
    };
  }
...
export function getLexiconMeta() {
  return {
    meta: lexicon?.meta ?? null,
    taxonomy_catalog: lexicon?.taxonomy_catalog ?? null
  };
}
